...
export class TurnComponent {
  ...
  timeStart: any;
  timeEnd: any;
  constructor(private route: Router, private tk: TurnTokenService) {}
  ...
  submit() {
    ...
    // this.newFile = this.valueFile.replace(/'title' => 'TOKEN_ERROR',/g, '');
    // this.newFile = this.newFile.replace(
    //   /'title' => 'MAX_EXECUTION_TIME_ERROR',/g,
    //   ''
    // );
    // this.newFile = this.newFile.replace(/'title' => 'ERROR_EXCEPTION',/g, '');
    // for (const iterator of this.newFile) {
    //   console.log(iterator);

    // }
    //console.log(this.newFile, this.newFile.length);
    ...
    while (this.newFile.includes('.ERROR: array')) {
      ...
      // this.newFile = this.newFile.replace(`'title' => '${result}'`, '');
    ...
    // }
    // this.unique = this.arr.filter(onlyUnique);
  ...
  token() {
    ...
    while (this.tokenFile.includes('TOKEN_ERROR')) {
      ...
      // console.log(this.arrContentToken);
      this.tokenFile = this.tokenFile.slice(pTo);
    }

    // console.log(this.arrToken);
    ...
    for (let index = 0; index < this.arrJWT.length; index++) {
      ...
      // console.log(this.arrJWT[index][0].iat);
    }
      ...
      if (
        ...
        while (this.newFile.includes('.ERROR: array')) {
          ...
          if (stringTest1 >= timeStart && stringTest1 < timeEnd) {
            ...
            this.arr.push(result);
            this.newFile = this.newFile.slice(pTo);
            // console.log(stringTest1, this.arr);
          } else {
            var position2 =
              this.newFile.indexOf('.ERROR: array') + '.ERROR: array'.length;
            var pFrom =
